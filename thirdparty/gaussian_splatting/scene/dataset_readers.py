...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, near, far, startime=0, duration=50):
    ...
    originnumpy = os.path.join(os.path.dirname(
        os.path.dirname(images_folder)), "poses_bounds.npy")
    with open(originnumpy, 'rb') as numpy_file:
        ...
        # note hard coded to half of the original image size
        imageH = int(H//2)
        imageW = int(W//2)
    ...
    for i in range(len(sortedtotalcamelist)):
        # map each cam with a number
        sortednamedict[sortedtotalcamelist[i]] = i
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        for j in range(startime, startime + int(duration)):
            image_path = os.path.join(
                images_folder, os.path.basename(extr.name))
            ...
            if j == startime:
                ...
                cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path, image_name=image_name,
                                      width=width, height=height, near=near, far=far, timestamp=(j-startime)/duration, pose=1, hpdirecitons=1, cxr=0.0, cyr=0.0)

            else:
                cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path, image_name=image_name, width=width,
                                      height=height, near=near, far=far, timestamp=(j-startime)/duration, pose=None, hpdirecitons=None, cxr=0.0, cyr=0.0)
...
def readColmapCamerasTechnicolor(cam_extrinsics, cam_intrinsics, images_folder, near, far, startime=0, duration=50):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        for j in range(startime, startime + int(duration)):
            ...
            if j == startime:
                cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path, image_name=image_name,
                                      width=width, height=height, near=near, far=far, timestamp=(j-startime)/duration, pose=1, hpdirecitons=1, cxr=cxr, cyr=cyr)
            else:
                cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path, image_name=image_name, width=width,
                                      height=height, near=near, far=far, timestamp=(j-startime)/duration, pose=None, hpdirecitons=None,  cxr=cxr, cyr=cyr)
            cam_infos.append(cam_info)
    sys.stdout.write('\n')
    return cam_infos
...
def readColmapCamerasMv(cam_extrinsics, cam_intrinsics, images_folder, near, far, startime=0, duration=50):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        if extr.name == "cam00.png":
            ...
            if cyr != 0.0:
                cxr = cxr
                cyr = cyr
                projection_matrix = getProjectionMatrixCV(
                    znear=0.01, zfar=100.0, fovX=FovX, fovY=FovY, cx=cxr, cy=cyr).transpose(0, 1).cuda()
            else:
                projection_matrix = getProjectionMatrix(
                    znear=0.01, zfar=100.0, fovX=FovX, fovY=FovY).transpose(0, 1).cuda()
    ...
    for i in range(240):
        ...
        newcameracenter = camera_center + radiace * \
            torch.Tensor((np.cos(theta), np.sin(theta),
                         2.0 + 2.0*np.sin(theta))).cuda()
        ...
        time = (i)/240
        cam_info = CameraInfo(uid=uid, R=newR, T=newt, FovY=FovY, FovX=FovX, image=None, image_path=None, image_name=image_name,
                              width=width, height=height, near=near, far=far, timestamp=time, pose=1, hpdirecitons=0, cxr=0.0, cyr=0.0)
...
def readColmapCamerasImmersive(cam_extrinsics, cam_intrinsics, images_folder, near, far, startime=0, duration=50):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        for j in range(startime, startime + int(duration)):
            ...
            rawvideofolder = os.path.join(
                parentfolder, os.path.basename(image_name))
...
def readColmapSceneInfoImmersive(path, images, eval, llffhold=8, multiview=False, duration=50, testonly=False):
    ...
    if testonly:
        cam_infos_unsorted = readColmapCamerasImmersiveTestonly(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(
            path, reading_dir), near=near, far=far, startime=starttime, duration=duration)
    else:
        cam_infos_unsorted = readColmapCamerasImmersive(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(
            path, reading_dir), near=near, far=far, startime=starttime, duration=duration)
    cam_infos = sorted(cam_infos_unsorted.copy(), key=lambda x: x.image_name)

    if eval:
        # + cam_infos[:duration] # for demo only
    ...
    totalply_path = os.path.join(
        path, "sparse/0/points3D_total" + str(duration) + ".ply")
    ...
    if not os.path.exists(totalply_path):
        ...
        for i in range(starttime, starttime + duration):
            thisbin_path = os.path.join(
                path, "sparse/0/points3D.bin").replace("colmap_" + str(starttime), "colmap_" + str(i), 1)
            xyz, rgb, _ = read_points3D_binary(thisbin_path)

            totalxyz.append(xyz)
            totalrgb.append(rgb)
            totaltime.append(
                np.ones((xyz.shape[0], 1)) * (i-starttime) / duration)
...
def readColmapSceneInfoMv(path, images, eval, llffhold=8, multiview=False, duration=50):
    ...
    cam_infos_unsorted = readColmapCamerasMv(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(
        path, reading_dir), near=near, far=far, startime=starttime, duration=duration)
    ...
    txt_path = os.path.join(path, "sparse/0/points3D.txt")
    totalply_path = os.path.join(
        path, "sparse/0/points3D_total" + str(duration) + "_mv.ply")
...
def readColmapSceneInfo(path, images, eval, llffhold=8, multiview=False, duration=50):
    ...
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(
        path, reading_dir), near=near, far=far, startime=starttime, duration=duration)
    cam_infos = sorted(cam_infos_unsorted.copy(), key=lambda x: x.image_name)
...
def readColmapSceneInfoTechnicolor(path, images, eval, llffhold=8, multiview=False, duration=50):
    ...
    near = 0.01
    far = 100
    cam_infos_unsorted = readColmapCamerasTechnicolor(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(
        path, reading_dir), near=near, far=far, startime=starttime, duration=duration)
    cam_infos = sorted(cam_infos_unsorted.copy(), key=lambda x: x.image_name)
    ...
    if eval:
        ...
        train_cam_infos = [
            _ for _ in cam_infos if not cam10.match(_.image_name)]
        ...
        if len(test_cam_infos) > 0:
            ...
            print(
                "do custom loader training, select first cam as test frame: ", first_cam)
            cam_infos = natsort.natsorted(
                cam_infos_unsorted.copy(), key=lambda x: x.image_name)
            train_cam_infos = [
                _ for _ in cam_infos if first_cam not in _.image_name]
            test_cam_infos = [
                _ for _ in cam_infos if first_cam in _.image_name]
    else:
        train_cam_infos = cam_infos
...
def readColmapCamerasImmersivev2(cam_extrinsics, cam_intrinsics, images_folder, near, far, startime=0, duration=50):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        for j in range(startime, startime + int(duration)):
            ...
            if j == startime:
                cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path, image_name=image_name, width=width,
                                      height=height, near=near, far=far, timestamp=(j-startime)/duration, pose=1, hpdirecitons=hpdirecitons, cxr=cxr, cyr=cyr)
            else:
                cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path, image_name=image_name, width=width,
                                      height=height, near=near, far=far, timestamp=(j-startime)/duration, pose=None, hpdirecitons=None,  cxr=cxr, cyr=cyr)
            cam_infos.append(cam_info)
